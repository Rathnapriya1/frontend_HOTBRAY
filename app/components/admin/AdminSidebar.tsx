        ...
        <div className="inline-flex h-full flex-col gap-5 border-r border-slate-200 sm:min-w-60">
            <div className="flex flex-col gap-3 justify-center items-center pt-8 max-sm:hidden">
                <div className="w-14 h-14 bg-slate-300 rounded-full flex items-center justify-center">
                    <span className="text-slate-600 text-sm">DGS</span>
                </div>
                <p className="text-slate-700">Hi, DGSTECH</p>
            </div>
            ...
            <div className="max-sm:mt-6">
                {sidebarLinks.map((link, index) => (
                    ...
                    >
                        ...
                        {pathname === link.href && (
                            <span className="absolute bg-blue-500 right-0 top-1.5 bottom-1.5 w-1 sm:w-1.5 rounded-l"></span>
                        )}
                    </Link>
                ))}
            </div>
